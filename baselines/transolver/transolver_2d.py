...
class Transolver2D(nn.Module):
    def __init__(self, space_dim=1, n_layers=5, n_hidden=256, dropout=0.0, n_head=8,
                 ...
                 slice_num=32, ref=8, unified_pos=False, H=85, W=85):
        super().__init__()
    ...
    def forward(self, x, fx, T=None):
        if self.unified_pos:
            pos = self.get_grid(x.shape[0])
            x = pos.repeat(x.shape[0], 1, 1, 1).reshape(x.shape[0], self.H * self.W, self.ref * self.ref)
        if fx is not None:
            fx = torch.cat((x, fx), -1)
